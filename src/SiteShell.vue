...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('manuscripts.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('manuscripts.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('manuscripts.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
